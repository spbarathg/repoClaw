 ...
 * Role: Classifies build failures using deterministic heuristics first.
 * AI is a constrained fallback used ONLY when heuristics return UNKNOWN with low match strength.
 * Architecture: Heuristic-first, AI-fallback, provider-agnostic.
 */
...
// Tier 1 (0.90): Highly specific matches — exact error codes or narrow signatures
...
const MATCH_STRENGTH_TIERS: Record<string, number> = {
  'INFRASTRUCTURE_FAILURE': 0.90,
  'OUT_OF_MEMORY': 0.88,
  'TYPESCRIPT_FAILURE': 0.85,
  'TYPESCRIPT_CONFIG_FAILURE': 0.85,
  'VITE_CONFIG_FAILURE': 0.82,
  'PYTHON_NATIVE_TOOLCHAIN_FAILURE': 0.82,
  'JAVA_COMPILE_FAILURE': 0.80,
  'PHP_PACKAGE_FAILURE': 0.78,
  'PERMISSION_FAILURE': 0.85,
  'SYNTAX_FAILURE': 0.83,
  'MISSING_DEPENDENCY': 0.78,
  'DEPENDENCY_CONFLICT': 0.80,
  'NETWORK_FETCH_FAILURE': 0.75,
  'BUILD_SCRIPT_MISSING': 0.88,
  'MISSING_ENV': 0.72,
  'RUNTIME_VERSION_MISMATCH': 0.75,
  'BUNDLER_FAILURE': 0.70,
  'BUILD_FAILURE': 0.65,
  'MISSING_FILE': 0.70,
  'UNKNOWN': 0.30,
};

const SEVERITY_MAP: Record<string, ErrorCategory['severity']> = {
...
// Categories where retry is never viable
const NON_RETRYABLE = new Set([
  'INFRASTRUCTURE_FAILURE', 'UNKNOWN', 'SYNTAX_FAILURE',
  'PERMISSION_FAILURE', 'OUT_OF_MEMORY'
]);

// ━━━━━━━━━━━━━━━━━━━━ HEURISTIC CLASSIFIER ━━━━━━━━━━━━━━━━━━━━
function heuristicClassify(stdout: string, stderr: string): ErrorCategory {
  ...
  // === PYTHON NATIVE TOOLCHAIN ===
    ...
    probableCause = 'A native compilation toolchain (gcc/clang/make) is missing. A Python package requires C/Fortran compilation from source.';
  ...
  } else if (stderrLc.includes('tsconfig') || stderrLc.includes('ts18003') || stderrLc.includes('no inputs were found') || (stderrLc.includes('typescript') && stderrLc.includes('config')) || (stdoutLc.includes('the typescript compiler') && stdoutLc.includes('common commands'))) {
  ...
  let matchStrength = MATCH_STRENGTH_TIERS[category] ?? 0.50;
  // Bonus: if we extracted a concrete package name, bump strength
  if (missingPackageName && missingPackageName.length > 0) matchStrength = Math.min(matchStrength + 0.05, 0.95);

  return {
    category,
    severity: SEVERITY_MAP[category] ?? 'high',
    probableCause,
    matchStrength,
    classificationSource: 'heuristic',
    suggestedFix,
    missingPackageName,
    retryRecommended: !NON_RETRYABLE.has(category)
  };
}

// ━━━━━━━━━━━━━━━━━━━━ AI FALLBACK CLASSIFIER ━━━━━━━━━━━━━━━━━━━━
async function aiFallbackClassify(stdout: string, stderr: string): Promise<ErrorCategory | null> {
  if (!config.geminiApiKey) return null;

  try {
    const promptPayload = `${ERROR_CLASSIFICATION_PROMPT}\n\nSTDOUT:\n${stdout.slice(-5000)}\n\nSTDERR:\n${stderr.slice(-5000)}`;
    ...
    if (response.data?.candidates?.[0]?.content?.parts?.[0]?.text) {
      ...
      // Cap AI match strength at 0.75 — never higher than a strong heuristic match
      return {
        category: parsed.category || 'UNKNOWN',
        severity: parsed.severity || 'high',
        probableCause: parsed.probableCause || 'Unknown',
        matchStrength: Math.min(parsed.confidence || 0.5, 0.75),
        classificationSource: 'ai_fallback' as const,
    ...
    logger.warn('AI fallback classification failed', { error: apiErr.message });
  }
...
export const errorClassifier = async (stdout: string, stderr: string): Promise<ErrorCategory> => {
  ...
  // STEP 1: Always run heuristics first (deterministic)
  const heuristicResult = heuristicClassify(stdout, stderr);

  // STEP 2: If heuristics produced a known category, use it directly
  if (heuristicResult.category !== 'UNKNOWN') {
    logger.info(`Classification: Heuristic -> ${heuristicResult.category} (match strength: ${heuristicResult.matchStrength})`);
    return heuristicResult;
  }

  // STEP 3: AI fallback ONLY for UNKNOWN with low match strength
  logger.info('Heuristic returned UNKNOWN — attempting AI fallback classification');
  const aiResult = await aiFallbackClassify(stdout, stderr);
  if (aiResult && aiResult.category !== 'UNKNOWN') {
    logger.info(`Classification: AI Fallback -> ${aiResult.category} (match strength: ${aiResult.matchStrength})`);
    return aiResult;
  }

  // STEP 4: Return heuristic UNKNOWN as final answer
  logger.info('Classification: UNKNOWN — no heuristic or AI match');
  return heuristicResult;
};
